...
db_path = 'E:\\crio.do WOD\\New folder\\projects\\api\\test.db'
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not helper.checkUrl(url):
                ...
                c.execute(
                    "INSERT into memes (name, caption, url) values (?,?,?)", (name, caption, url))

                # Inserting in Global List too
                if ult == 1:
                    res.insert(0, {"id": (len(res) + 1), "name": name,
    ...
    if request.method == 'PATCH':
        ...
        try:
            ...
            if caption and (not url):
                c.execute(
                    "UPDATE memes SET caption = ? WHERE rowid = ?", (caption, id))
                if ult == 1:
            ...
            # Check if Url is available already in database
                ...
                if caption and url:
                    c.execute(
                        "UPDATE memes SET caption = ?, url = ? WHERE rowid = ?", (caption, url, id))
                    ...
                    c.execute(
                        "UPDATE memes SET url = ? WHERE rowid = ?", (url, id))
                    # Updating Global List if exist
                    if ult == 1:
                        res[len(res)-id]['url'] = url
