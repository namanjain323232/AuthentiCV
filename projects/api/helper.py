...
def get_all():
    ...
    try:
        ...
        for i in range(len(rows)):
            temp = {"id": f"{rows[i][0]}", "name": f"{rows[i][1]}",
                    "caption": f"{rows[i][2]}", "url": f"{rows[i][3]}"}
            res.append(temp)
...
def extract_json_param(req, req_method):
    if req_method == 'POST':
        ...
        try:
            caption = req.json['caption']
            url = req.json['url']
            if (caption == "" and url == "") or (caption == None and url == None):
                raise Exception("Both parameters are null")
            status_code = 200
        except Exception:
            caption = url = None
        finally:
